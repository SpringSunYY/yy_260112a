  ...
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="lectureList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="编号" align="center" v-if="columns[0].visible" prop="id" />
        ...
        <template slot-scope="scope">
          <span>{{ parseTime(scope.row.lectureStartTime, '{y}-{m}-{d}') }}</span>
        </template>
      </el-table-column>
        <el-table-column label="结束时间" align="center" v-if="columns[6].visible" prop="lectureEndTime" width="180">
        <template slot-scope="scope">
          <span>{{ parseTime(scope.row.lectureEndTime, '{y}-{m}-{d}') }}</span>
        </template>
      </el-table-column>
        <el-table-column label="参加人数" :show-overflow-tooltip="true" align="center" v-if="columns[7].visible" prop="peopleJoinNumber" />
        <el-table-column label="签到人数" :show-overflow-tooltip="true" align="center" v-if="columns[8].visible" prop="peopleSignNumber" />
        ...
        <el-table-column label="讲座描述" :show-overflow-tooltip="true" align="center" v-if="columns[10].visible" prop="description" />
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="教室" prop="classroomId">
          <el-input v-model="form.classroomId" placeholder="请输入教室" />
        </el-form-item>
        <el-form-item label="名称" prop="name">
        ...
        <el-form-item label="人数" prop="peopleNumberLimit">
          <el-input v-model="form.peopleNumberLimit" placeholder="请输入人数" />
        </el-form-item>
...
<script>
import { listLecture, getLecture, delLecture, addLecture, updateLecture } from "@/api/manage/lecture";
    ...
    return {
      ...
      columns: [
        { key: 0, label: '编号', visible: true },
          ...
          { key: 10, label: '讲座描述', visible: true },
